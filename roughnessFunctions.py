# -*- coding: utf-8 -*-
"""
Created on Thu Dec  6 14:17:09 2018
...
def meshEntropy(blade_verts, blade_colors, patchSize = 80):    
    ...
    blade_colors_hsv = rgb_to_hsv(blade_colors)
...
#        nearestNeighbors_intensity = (nearestNeighbors_colors[:,0] + nearestNeighbors_colors[:,1] + nearestNeighbors_colors[:,2])/3
        ...
        maskedColor = np.ones([len(currNdx),1])*currEntropy
        blade_colors_masked[currNdx] = maskedColor
    ...
    allCheckedNdx = np.array(allCheckedNdx)
...
#            continue
    ...
    #    currNormMag =np.linalg.norm(delta_norm)
        allDeltaNorms_magnitude.append(currNormMag)
        
        maskedColor = np.ones([len(currNdx),1])*currNormMag
        blade_colors_masked[currNdx] = maskedColor
...
def meshPointDensityAndColorUniformity(blade_verts, blade_colors, percentChange = [0.1,0.15,0.15], num_percentChange = 0.5, percentFromMaxNeighbour = 0.3):
    pcd = PointCloud()
...
#            if maxNeighbours < len(currNdx_large):
#                maxNeighbours = len(currNdx_large)
...
#        maxNeighbours_all.append(maxNeighbours)
    ...
    #numNeighbours = 0.5 * maxNeighbours    #50
...
#                nearestNeighbors_colors = blade_colors[currNdx_large].mean(axis=1)
#                
#            
#                curr_color = blade_colors[i].mean()
#                
#                bool_changeArray = np.abs(( nearestNeighbors_colors -  curr_color )/ curr_color )<percentChange
                
                
#                neighbors_color_hsv = np.zeros_like(blade_colors[currNdx_large])
#            
#                for k in range(0, len(currNdx_large)):
#                    neighbors_color_hsv[k] = colorsys.rgb_to_hsv(blade_colors[currNdx_large][k,0],blade_colors[currNdx_large][k,1],blade_colors[currNdx_large][k,2])
#   
                
#                COLOR STUFF
#                neighbors_color_hsv = rgb_to_hsv(blade_colors[currNdx_large])
#                    
#                curr_color = colorsys.rgb_to_hsv(blade_colors[i,0],blade_colors[i,1],blade_colors[i,2])
#                
#                bool_changeArray = np.logical_and(np.abs(( neighbors_color_hsv[:,0] -  curr_color[0] + 0.000001 )/ (curr_color[0]+ 0.000001) )<percentChange[0], np.logical_and(np.abs(( neighbors_color_hsv[:,1] -  curr_color[1]+ 0.000001 )/ (curr_color[1]+ 0.000001) )<percentChange[1] , np.abs(( neighbors_color_hsv[:,2] -  curr_color[2]+ 0.000001 )/ (curr_color[2]+ 0.000001) )<percentChange[2])  )
#            
#                
#                isEnough = np.count_nonzero(bool_changeArray)/len(bool_changeArray)
                
#                if isEnough > num_percentChange:
